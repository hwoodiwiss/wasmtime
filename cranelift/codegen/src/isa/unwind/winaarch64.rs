//! Windows aarch64 ABI unwind information.
...
/// Maximum (inclusive) size of a "small" stack allocation (alloc_s)
const SMALL_ALLOC_MAX_SIZE: u32 = 512;
/// Maximum (inclusive) size of a "medium" stack allocation that can represented in 16-bits (alloc_m)
const MEDIUM_ALLOC_16BIT_MAX_SIZE: u32 = 32768;
/// Maximum (inclusive) size of a "large" stack allocation that can represented in 16-bits
const LARGE_ALLOC_16BIT_MAX_SIZE: u32 = 268435456;
...
///
/// See: <https://learn.microsoft.com/en-us/cpp/build/arm64-exception-handling#unwind-codes>
/// Only what is needed to describe the prologues generated by the Cranelift ARM64 ISA are represented here.
#[allow(dead_code)]
    ...
    fn node_count(&self) -> usize {
        match self {
            Self::StackAlloc { size, .. } => {
                if *size <= SMALL_ALLOC_MAX_SIZE {
                    1
                } else if *size <= MEDIUM_ALLOC_16BIT_MAX_SIZE {
                    2
                } else {
                    4
                }
...
pub(crate) enum MappedRegister {
    // TODO: Add AARCH64 Registers to be mapped for unwind
}
